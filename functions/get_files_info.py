...
def get_files_info(working_directory, directory="."):
    ...
    full_directory_path = os.path.abspath(os.path.join(absolute_working_dir, directory))
    
    #First check for proper directory inputs 
    if not full_directory_path.startswith(absolute_working_dir):
        return f'Error: Cannot list "{directory}" as it is outside the permitted working directory'
    if not os.path.isdir(full_directory_path): #Check that the path is a directory
    ...
    try:
        ...
        files = sorted(os.listdir(full_directory_path))
        files_info = []
        for file in files:
